...
public partial class AccountDetailsWindow : Window
{
    ...
    {
        var error = await _main.SyncAsync(_details.Id);
        if (!string.IsNullOrWhiteSpace(error)) MessageBox.Show(this, error, "Unable to sync account", MessageBoxButton.OK, MessageBoxImage.Warning);
        await ReloadAsync();
    }
    private async void Edit_Click(object sender, RoutedEventArgs e)
    {
        var account = await _main.GetAccountAsync(_details.Id, includePassword: true); if (account is null) return;
        ...
        if (dialog.Result is not null) await _main.SaveAccountAsync(dialog.Result);
        await ReloadAsync();
    }
    private async Task ReloadAsync()
    {
        var account = await _main.GetAccountAsync(_details.Id); if (account is null) { Close(); return; }
        _details.Dispose(); DataContext = _details = new(account, _artwork, _main.Settings);
    }
    private void MainPropertyChanged(object? sender, PropertyChangedEventArgs e) { if (e.PropertyName == nameof(MainViewModel.IsVault) && !_main.IsVault) Close(); }
